...
const showQuizGallery = () =>
{
    for (let i = 0; i < data.length; i++)
    {
        ...
        quizContainer.append(quizImage);
        quizImage.src = `${data[i].url}`;
        quizContainer.append(quizText);
        quizText.textContent = `${data[i].question}`;
        quizContainer.append(answerBtnContainer);
        
        for (let j = 0; j < data[i].choice.length; j++)
        {   
            const quizButton = document.createElement('button');
            answerBtnContainer.append(quizButton);
            quizButton.textContent = `${data[i].choice[j]}`;
        };
    };
};
...
const allBtnContainer = document.querySelectorAll('div');
...
const checkAnswer = () =>
{
    ...
    // check which answers are available 
    for (let i = 0; i < allBtnContainer.length; i++)
    {
        allBtnContainer[i].addEventListener('mouseenter', () =>
        {
          answer = data[i].answer.toString();
          siblingButton = allBtnContainer[i].querySelectorAll('button');
        });
    };
    ...
    allButtons.forEach((button) =>
    {
        ...
        {
          // disable other choices when choice made + show correct answer
          siblingButton.forEach((button) => 
          {
